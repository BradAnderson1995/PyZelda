...
class Tileset:
    ...
    def readTiles(self):
        """
        This method reads the tiles from the tileset and also fills up the
        tile dictionary.
        """
        ...
        for y in range(TilesetH):
            for x in range(TilesetW):
                ...
                if tile_str not in self.TileDict:
                    self.TileDict[tile_str] = len(self.List) - 1

    def findTile(self, tileImage):
        """
        This method returns the tile index for the given tile image if it is
        part of this tileset, and returns 0 if the tile could not be found.
        Constant complexity due to dictionary lookup.
        """
        tile_str = tileImage.tostring()
        if tile_str in self.TileDict:
            return self.TileDict[tile_str] + 1
        else:
            return 0
...
class TileMap:
    ...
    def printProgress(self, percentage):
        """
        This function prints the percentage on the current row after erasing
